...
def animate_foodweb(scor_file_in, gif_file_out, fps=10, anim_len=2, trails=6, min_node_radius=0.5, max_node_radius=10, min_part_num=2, max_part_num=150, map_fun=np.sqrt, if_imports=True, if_exports=False, how_to_color='tl', cmap=sns.color_palette(), max_luminance=0.85):
    ...
    def panimate(frame):  # creates one frame of the animation
        ...
        layer(frame, particles, netIm, 1, t=interval+shadeStep*trails, how_to_color=how_to_color,
              max_width=max_width, particle_size=particle_size)  # print the present positions of the particles
        #add vertices
        fig = plt.gcf()
        ax = fig.gca()
        add_vertices(ax, netIm.nodes, r_min=min_node_radius, r_max=max_node_radius,
                     font_size=font_size, alpha_=0.95, map_fun=map_fun)
        ...
        if trails > 0.0:
            for i in range(trails):  # shadow: alpha decreasing from 1 for i=0 to 0.2 at trails
                ...
                layer(frame, particles, netIm, shadow_alpha, t=-shadeStep,
                      how_to_color=how_to_color, max_width=max_width, particle_size=particle_size)
                T += shadeStep
    ...
    netIm = readNetImage('../examples/'+scor_file_in, 80, min_part_num=min_part_num,
...
# plt.close()
    animate(gif_file_out, panimate, frames=frameNumber, interval=interval, figsize=(
        20, 20), dpi=dpi, fps=fps)  # figsize=(12, 12)) #default dpi=100 #
